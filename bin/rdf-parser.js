...
function _parseRecord(record, callback) {
  var response = {
    id: null,
    formats: [],
    //downloads: 0,
  ...
  parseString(record, function(err, jsonRecord) {
    ...
    try {
      response.id = jsonRecord['rdf:RDF']['pgterms:ebook'][0]['$'][
        'rdf:about'
      ].replace('ebooks/', '');
      if (jsonRecord['rdf:RDF']['pgterms:ebook'][0]['dcterms:language'][0]) {
        if (
          jsonRecord['rdf:RDF']['pgterms:ebook'][0]['dcterms:language'][0][
            'rdf:Description'
          ]
        ) {
      ...
      if (jsonRecord['rdf:RDF']['pgterms:ebook'][0]['dcterms:title']) {
        response.title =
          jsonRecord['rdf:RDF']['pgterms:ebook'][0]['dcterms:title'];
      }

      var creatorNode = null;
      if (jsonRecord['rdf:RDF']['pgterms:ebook'][0]['dcterms:creator']) {
        creatorNode =
          jsonRecord['rdf:RDF']['pgterms:ebook'][0]['dcterms:creator'];
      } else {
        ...
        if (marcrelKeys.length > 0) {
          creatorNode =
            jsonRecord['rdf:RDF']['pgterms:ebook'][0][marcrelKeys[0]];
        }
      }
      ...
      try {
        if (creatorNode) {
          response.author =
            creatorNode[0]['pgterms:agent'][0]['pgterms:name'][0];
        } else {
      ...
      if (jsonRecord['rdf:RDF']['pgterms:ebook'][0]['dcterms:issued']) {
        response.issued =
          jsonRecord['rdf:RDF']['pgterms:ebook'][0]['dcterms:issued'][0]['_'];
      }

      if (jsonRecord['rdf:RDF']['pgterms:ebook'][0]['dcterms:hasFormat']) {
        var files =
          jsonRecord['rdf:RDF']['pgterms:ebook'][0]['dcterms:hasFormat'];
        for (var i = 0; i <= files.length - 1; i++) {
          var file = files[i];
          var format =
            file['pgterms:file'][0]['dcterms:format'][0]['rdf:Description'][0][
              'rdf:value'
            ][0]['_'];
          var tempFilePath = file['pgterms:file'][0]['$']['rdf:about'];
          var fileName = file['pgterms:file'][0]['$']['rdf:about']
            .replace('http://www.gutenberg.org/ebooks/', '')
            .replace('http://www.gutenberg.org/files/', '');
          ...
          if (tempFilePath.indexOf('http://www.gutenberg.org/dirs') > -1)
            outputContainer.specialPath = tempFilePath.replace(
